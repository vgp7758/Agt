...
| [features/wiki-auto-maintenance](features/wiki-auto-maintenance.md) | wiki_auto_maintenance：update_wiki → build_commit_msg → snap_before → diff_wiki → commit_wiki（git_commit 节点），自动维护并 git 提交推送 wiki | 改 wiki 维护流程 / 调 commit 节点 |
...
| [releases/v0.18.2](releases/v0.18.2.md) | v0.18.2 发布记录：唤醒链路根因修复、stdin 通道、/api/status、async 元信息、气泡折叠、wiki 自动提交（含提交失败修复） | 查版本交付内容 / 发布流程 |
...
- **wiki 提交失败修复**（2026-08）：commit_wiki 从 run_shell 改 **git_commit 节点**（subprocess 列表参数规避 shell 多行转义），引入 **snap_before/diff_wiki** 生成变更清单，自动追加 Co-authored-by——详见 [wiki-auto-maintenance](features/wiki-auto-maintenance.md#提交失败问题2026-08-修复)
...
- wiki_auto_maintenance 工作流在 update_wiki 后接 build_commit_msg（text 节点，将报告摘要注入 commit message）→ snap_before（快照）→ diff_wiki（生成变更清单）→ commit_wiki（**git_commit 节点**，列表参数规避 shell 多行转义，自动追加 Co-authored-by），按清单 git add/commit/push `.agent/wiki/`，每条提交携带具体内容摘要——详见 [wiki-auto-maintenance](features/wiki-auto-maintenance.md)
