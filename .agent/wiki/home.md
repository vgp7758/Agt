...
| [features/tool-externalization](features/tool-externalization.md) | 工具外置体系：tools/builtin/*.py + `agt_register(ctx)` 描述符 + **ctx 通用上下文注入（签名兼容传 cwd，外置件不依赖 import 时 Path.cwd()）** + `/reload tools` 热加载 + 随包副本 + **五外置件两形态（纯函数整体外置 / 注册外置+实现留框架）** | 加新内置工具（零框架改动） |
