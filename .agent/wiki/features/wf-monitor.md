...
> src/workflow.py（`_WF_RUNS` 注册表）+ src/agent.py（钩子 run_id 生成）+ src/server.py（3 路由）+ src/static/wf_monitor.html（观测页）+ src/static/index.html（执行中行可点击）。2026-08-20 新建，commit 8aeb21a。
...
`auto_wf_start` 事件带 `run_id` 时，「⏳ 工作流「xxx」执行中…」行**可点击**（虚线下划线 + pointer），`window.open('/wf/monitor?run='+encodeURIComponent(m.run_id))` 新标签打开——这是并行钩子 UI 的自然延伸（见 [user-interaction · 并行钩子 UI](user-interaction.md#并行钩子执行中状态跟踪修复2026-08-19)）。
...
- **/restart 后生效**：旧进程代码不含注册逻辑，「执行中」行不携带 run_id（不可点击）
...
- [用户交互](user-interaction.md)：并行钩子「执行中」行（现可点击打开观测页）
