...
#### 后记七：跳级修复——收益判定不阻断阶梯 + 按轮吃上限=超深段末端（2026-09-24，用户实锤，commit 0d1e7d3）

**用户实锤（2026-09-24）**：「基本所有的 session 触发顶线毕业时还都是好几档跳过[工具摘要档]直接进了[结构摘要档]，应该先进工具折叠档，不够再从工具折叠档前面划若干轮到结构摘要。」——862 轮的三级贯通代码其实还在，但被**两处缺口架空**：

| # | 缺口 | 后果 |
|---|---|---|
| ① | `_deepen_oldest_tier` 的「**收益 <2% → return False**」一刀切跳过 | 老档早压到 detail_base 下限（工具调用占比小）时单刀收益 <2% → **整个中间级被弃** → 压力循环 break → 直接折叠大刀（跳级）。这个早退条件正是压缩阶梯三改开发轮为防「每刀全量重渲染卡死」加的保守闸（见上节"三个诚实的发现"第 3 条）——防卡死的闸在真实分布下变成了跳级通道 |
| ② | `_next_fold_target` 按轮吃**无档位上限** | 碎刀从 fc+1 逐轮吃到达标——**越过超深段**把还在文字档的轮直接折进结构摘要（用户观察的"好几档跳过"） |

**修复**（src/session.py，commit `0d1e7d3`）：

- **① 删收益跳过**：est 仍超 target 时**永不因小收益放弃推进**——每刀推最老档跨入超深（结构正确），小收益也推；循环回来自然轮到次老档，**阶梯逐级渐进**。达标由调用方循环条件保证（est ≤ target 即 break，不会多推）
- **② 按轮吃的上限 = 超深段末端**（`bs[-max_level]+1`）：fold_deep_tools 开时只吃**工具折叠档**的轮；吃满超深段仍不达标 → 返回 cap（"超深已折尽"信号）——下轮压力循环先推进②扩大工具折叠档再折 → **跨触发渐进**，不再单轮强吃到位

正是用户预期的完整阶梯：**升档 → 先整档进工具折叠档 → 还不够才从工具折叠档前面划若干轮到结构摘要**。

**验证**（selftest 场景矩阵 17/17 全绿）：新增**跳级黄金断言**——hook 两个折叠目标函数抓「每次折叠决策瞬间」的边界状态（外部抓不到：prune 会清 `< fc` 的边界）：`被折轮全部来自工具折叠档（越界决策=[]）` ✓ + `fc 处达标 或 fc=超深段末端（跨触发渐进）` ✓。**修复前的实证**就在失败记录里：fc=30 但被折轮含文字档轮 `[0,1,2,3,4]`（正是用户观察的现象）。其余 15 项（阶梯顺序/推老档精确行为/达标即停/fold_deep 关不回归/prune/restore）保持通过。`/restart` 生效。
