...
图片转 Markdown（2026-09，用户请求，注册 version 1→2）**：白名单 `_ZAI_FT` = 文档 11 种（pdf/docx/doc/xls/xlsx/ppt/pptx/csv/txt/md/html）+ **图片 16 种**（png/jpg/jpeg/bmp/gif/webp/heic/heif/jp2/eps/icns/im/pcx/ppm/tiff/xbm）。别名归一 `_FT_ALIAS`（tif→tiff/htm→html/markdown→md/jpe→jpg）；不支持的类型**明确报错 + 列全清单**（不再静默失败）。`file_type` 从必填放宽为**可选**（默认按扩展名自动识别 + 别名归一）。**实测修正口径：图片解析不是纯 OCR，是「图片转 Markdown」**——含文字封面图 → 标题/正文全转写 + 图示部分图片引用保留；纯 logo 无文字 → 只回引用不硬编（详见 [zai-tools](features/zai-tools.md)）
