...
**排障经验（2026-09，commit c307a08）**：该端点是 server.py 启动时装配的——**进程跑旧代码时 `/api/wf/nodes` 只回核心节点、插件全缺席** → 编辑器里插件节点 hover 无 tooltip（现象像"功能没有"，实为旧进程在跑）。改了插件 `.py` 的 desc 或新增插件后需 **/restart** 才同步到编辑器（Ctrl+F5 只管前端注入，管不到后端聚合）。编辑器侧另有兜底：目录/工具 schema 双未命中时回显类型名，不静默空 title（详见 [编辑器 UX · 批次九 nodeDesc 兜底链](../features/editor-ux-improvements.md)）。
...
## 第四批：intent_nano——判别式意图路由（2026-09-22，commit 2aabfaa）
