...
- **合成记录键名契约**（2026-09-11 修复，v0.26.6，commit `1d47b37`）：seed 四键 `{tool, args, result, reasoning}`——`tool` 是消费侧 `_seed_steps` 读的键；此前写成 `"name"` → 工具名恒空 → 通知轮投影成**无名无工具形态**（看起来像纯 user 通知，用户观察触发修复）。与 service_exit 同款契约，见 [用户交互 · 键名漂移修复](user-interaction.md#键名漂移修复bg_task-合成记录-name--tool2026-09-11用户观察触发)
