...
快照包含 **19 个顶层字段** + **3 个嵌套数组**，覆盖实例运行时的关键维度：

- 顶层字段：实例元信息、运行状态、配置摘要等 19 个字段
- 嵌套数组：3 个，分别对应不同维度的列表型状态（如 Agent 实例列表等）

> 具体字段名与结构以 `src/server.py` 中端点实现为准。
>
> **`hold` 字段（2026-09-21 新增，随挂起功能）**：`bool(getattr(agent, "_hold", False))`——实例是否处于挂起态（`/hold on` 后 react 在下一步开始前暂停）。供外部监控/脚本识别实例是否被手动暂停，与 [挂起功能](user-interaction.md#⏸-挂起hold-onoff--webui-按钮--apihold-三通道2026-09-21用户提案) 的 `/api/hold` 写通道配套（本端点只读其状态）。
