...
agent_watch.py（本机常驻 · 独立进程，不在任何实例的服务树里）
  ...
  └─ 每 900s → POST {各实例}/api/status（timeout 8s；配 token 的实例 401 时带 X-Cb-Token 重试）
       ...
       → 与状态文件上一轮对比，diff_events 分级：
            mail_ev（只有轮数变化）→ SMTP_SSL 465 发 QQ 邮箱（触发实例附「局域网 + 公网 /port-N」地址块）
            note_ev（上下线/会话切换/inbox）→ 只更新状态文件 + 落日志，不发邮件（2026-09-26 裁定）
       → 每轮落一行诊断日志 ~/.agt/agent_watch.log（探到 N · state M · 事件段 K · 状态事件 N）
```
...
指纹 = `(alive, session, turns, busy, inbox)`（`fingerprint()`）。`diff_events()` 返回**两级事件** `(mail_ev, note_ev)`（2026-09-26 用户裁定拆分，见「邮件触发收窄」节）：

**mail_ev——触发邮件（唯一触发源 = 完成轮数变化）**：

| 事件 | 触发 |
|------|------|
| 💬 新增 N 轮回答 | turns 增加（主事件——实例产出新 answer） |
| 🔁 轮数回退 | turns 减少（重开会话/回溯） |

**note_ev——只记状态不发邮件**（落诊断日志 `~/.agt/agent_watch.log`）：

| 事件 | 触发 |
|------|------|
| 👀 首次纳入监视（基线，只记状态） | state 里不存在的实例（新实例首探）；反复出现 = 自动发现抖动（已修，见「误报修复」节） |
| 🟢 上线 | 此前不可达 → 可达 |
| 🔴 不可达（附错误） | 可达 → 不可达；**持续离线不重复报** |
| 🔁 会话切换 | session 名变 |
| 📥 inbox +N | 排队消息增加 |
| ~~⏳ 开始忙碌 / ✅ 空闲~~ | busy 翻转——**2026-09-22 · 四降噪移除**（busy 仍留在指纹元组，仅事件层不产出） |

邮件纪律：
- **只有轮数变化进邮件**；状态类变化只更新状态文件 + 落诊断日志（2026-09-26 裁定）
- 无轮数变化的实例不出现在邮件里；全员轮数无变化 → 整封跳过
...
## 邮件骚扰根因挖清：三进程并跑互相覆盖 state + busy 翻转降噪（2026-09-22 · 四，commit 6db27fc）

现象：`0a7f0e5` 修复后**仍每轮收「【Agent Watch】N 项变化」**。深挖出完整根因链——邮件骚扰是三个真凶叠加，最大的一个此前没被发现：

| # | 真凶 | 说明 | 处置 |
|---|---|---|---|
| 1（最大） | **3 个监视进程并存** | pid `19064 / 19452 / 31620` 同时在跑——历史多次 DETACHED 启动从不清场；轮询相位错开、**互相覆盖 state 文件**，指纹基准被搅乱 → 每轮都"有变化" | 全部清杀，收敛为**单进程（pid 4760）** |
| 2 | **busy↔空闲翻转被当事件** | 活跃实例（9000/8000）每 15 分钟随巡检在"忙/闲"间翻转，纯常态噪音，每翻一次报一次 | 从事件列表移除（`diff_events()` 不再产出 busy 事件行）；实质事件收敛为**上下线 / 会话切换 / 轮数变化 / inbox 积压**四类 |
| 3 | state 覆盖 + 自动发现抖动 | "暂时没扫到 ≠ 下线"，反复误判「首次纳入」 | 上轮已修（`0a7f0e5`，见上节） |

> 注：代码注释把本次降噪日期写成「2026-09-20」，实为 2026-09-22（笔误，不影响逻辑）。

验证（修复后连续两轮 `--once`）：

- 第 1 轮：1 项变化 → 邮件已发——**这封是真实的**：brick（CNB 容器）上线（agt-web 200 · turns=22）；已核实其 daemon 未跑，与本机生产 daemon 无抢消息冲突
- 第 2 轮：无变化 → 整封跳过 ✓✓
- 进程收敛确认：agent_watch 仅 1 个（pid 4760）

**定案语义**：从此没有实质变化（新 answer / 上下线 / 会话切换 / 消息积压）就不再发邮件——监视器只在真有事时打扰。

## 邮件触发收窄：只有轮数变化才发邮件（2026-09-26，用户裁定，commit e5e8b7e）

用户裁定最终口径：**状态类变化（上下线 / 会话切换 / inbox 积压）不值得邮件打扰**——上下线在自动发现体系里本就是常态（剧组开机关机、CNB 容器回收复活），inbox 积压多为后台通知自消化。收窄后（commit `e5e8b7e`）：

- `diff_events()` 返回 `(mail_ev, note_ev)` 两级：**mail_ev 只有轮数 ±**（💬 新增 N 轮回答 / 🔁 轮数回退）；note_ev 收纳全部状态类事件（🟢 上线 / 🔴 不可达 / 🔁 会话切换 / 📥 inbox +N / 👀 首次纳入——文案同步改为「基线，只记状态」）
- `run_once()` 分流：mail_ev 非空 → 发邮件（触发实例仍附完整状态行：session/轮数/busy/inbox/局域网+公网地址）；note_ev → `_note_lines` 汇总，**状态变化不消失**
- 诊断日志升级：每轮 `~/.agt/agent_watch.log` 除「探到 N · state M · 事件段 K」外追加**「状态事件 N」计数 + 逐行留痕**（`实例: 事件`）——「为什么没发」一查便知
- 全员轮数无变化 → 整封跳过（不变）；首轮只建基线不发信（不变）

验证（真实链路）：单测 4 场景（轮数涨→邮件 / 会话切换+inbox→记状态 / 下线→记状态 / 无变化→空）✓；真跑 `--baseline` 零事件零邮件 ✓；真跑 `--once` 抓到 main-9000 turns+1（上一轮对话本身）→ 正确发出唯一一封 ✓。

至此邮件语义三代演进：全量事件（2026-09-20 初版）→ 去 busy 翻转 + 四类实质事件（2026-09-22 · 四）→ **唯一触发源 = 完成轮数变化**（2026-09-26）。

## 部署清单（2026-09-20 · 二 实况）

**部署形态（2026-09-20 · 二）**：独立进程常驻（pid 19452，DETACHED 启动；2026-09-22 · 三 误报修复后重启为 pid 31620；同日 · 四 挖出**三进程并跑**实为骚扰元凶——全部清杀，收敛为单进程 **pid 4760**）——**不在 9000 的服务树里**，9000 `/restart` 不影响监视；代码已 commit（`feat(tools): agent_watch 自动发现 + cpolar /port-N 公网路由`，push 随下次提交带出；初版 pid 13852 已被本版取代）。
...
- **常驻只许一个进程**：历史多次 DETACHED 启动会并存——多进程轮询相位错开且互相覆盖 state → 指纹基准被搅乱、每轮误发（2026-09-22 · 四实锤：三进程并存即头号元凶）；重启/升级前先查杀旧进程再拉新
