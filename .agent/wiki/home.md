...
## 快速事实增补（2026-09-19 · 二 · QQ 邮箱「添加账户」问诊——扫码登录进行中）

- **QQ 邮箱「添加账户」问诊——授权码入口排查进行中（2026-09-19 · 二）**：用户报「邮箱网页版里添加账户找不到了，是不是要开会员」，主 Agent 用 playwright 开 `mail.qq.com` 查证——QQ 邮箱网页登录即要求**手机 QQ 扫码**（登录页截图 `qq_mail_login.png` 已存 workspace，等待用户扫码）。当前工作假设：**非会员问题**，QQ 邮箱新版 UI 把「POP3/SMTP/IMAP 服务 + 授权码」设置入口挪了位置，扫码进邮箱后去「设置 → 账户」确认。结论待登录后闭环（届时若确有实用结论再立专页）
