...
def _cmd_debug(ctx: CommandContext, args):
    ...
    t0 = _time.time()
    try:
        resp = agent.utility_client().chat(msgs, tools=agent._llm_tool_schemas(), scene="debug")
    except Exception as e:
...
def _cmd_context(ctx: CommandContext, args):
    ...
    # ② 分段估算（三级：内存 live → 旁车 sidecar（跨重启的最近真实投影+档位边界快照）→ 现算兜底）
