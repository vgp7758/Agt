...
agent_watch.py（本机常驻 · 独立进程，不在任何实例的服务树里）
  ├─ build_watches(prev_state)：每轮重建监视清单（沿用上轮发现过的端口——netstat 抖动不丢实例，见「误报修复」节）
  ...
  └─ 每 900s → POST {各实例}/api/status（timeout 8s；配 token 的实例 401 时带 X-Cb-Token 重试）
       ...
       → 每轮落一行诊断日志 ~/.agt/agent_watch.log（探到 N · state M · 事件段 K）
```
...
|------|------|
| 🟢 上线 | 此前不可达 → 可达 |
| 👀 首次纳入监视（基线） | state 里不存在的实例（新实例首探）；正常只出现一次，**反复出现 = 自动发现抖动误报**（已修，见「误报修复」节） |
...
## 误报修复：自动发现集合不稳定 →「首次纳入（基线）」反复触发邮件（2026-09-22 · 三，用户实锤，commit 0a7f0e5）

现象：用户收到的邮件里**所有实例轮数都没变化**——为什么还发？排查定性：事件不是「💬 新增 N 轮」，而是 **「👀 首次纳入监视（基线）」**——实例在 state 里"消失又重现"的假象，每轮重演：

```
① 某轮 netstat 偶发慢/失败或端口瞬时抖动 → 只探到部分实例
② state 文件被「本轮探到的」整体覆盖 → 没探到的实例从 state 里消失
③ 下一轮又被探到 → state 里找不到 → 判定「首次纳入（基线）」→ 发邮件
```

修复三件（commit 0a7f0e5）：

1. **state 合并保留**（`run_once`）：保存前把 `prev_state` 中本轮未探到的键原样落回 `new_state`——**"暂时没扫到" ≠ "实例下线"**；真下线仍走 🔴 语义（持续离线不重复报），不会因此多报
2. **`build_watches(prev_state)`**：签名加 `prev_state`，非空时把上轮发现过的端口并入本轮探测（键匹配 `port-(\d+)` 直取端口，其余按 `STATIC_META` 名字反查）——netstat 偶发失败也不丢实例
3. **诊断日志**：每轮往 `~/.agt/agent_watch.log` 追加一行：`[MM-DD HH:MM:SS] 探到 N 个 · state M 条 · 事件段 K`——「为什么发/没发」一查便知

验证（连续两轮 `--once`）：

- 第 1 轮：7 项变化 → 邮件已发（**合理**——state 里只有旧 4 条，自动发现新收编的实例确实首次纳入）
- 第 2 轮：同样 7 个实例、指纹一致 → **无变化，静默** ✓ 修复生效

收尾：常驻进程重启为 **pid 31620**（跑修复版，取代 pid 19452）。

## 部署清单（2026-09-20 · 二 实况）

**部署形态（2026-09-20 · 二）**：独立进程常驻（pid 19452，DETACHED 启动；2026-09-22 · 三 误报修复后重启为 pid 31620）——**不在 9000 的服务树里**，9000 `/restart` 不影响监视；代码已 commit（`feat(tools): agent_watch 自动发现 + cpolar /port-N 公网路由`，push 随下次提交带出；初版 pid 13852 已被本版取代）。
...
- **误报排查**：查 `~/.agt/agent_watch.log`（每轮一行：探到 N · state M · 事件段 K）；「👀 首次纳入（基线）」反复出现 = 自动发现抖动——已修（见「误报修复」节）
