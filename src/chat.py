...
def build_agent(mcp_mgr, *, on_event=None, snapshot_manager=None, verbose=True, workspace=WORKSPACE):
    ...
    # 多 agt 实例组网：remote_* 工具组。连接表持久化在【session 存档】（extra_state.remote_servers
    # ——2026-09-18 二轮裁定：跟 session 走，切会话=切组网、新会话空表；恢复在 set_session →
    # restore_runtime_state → restore_servers；旧全局 settings 份仅作一次性迁移源）。server_id
    # 路由在 Agent._exec_tool（工具执行统一入口），SYSTEM 清单由 {func:load_remote_instances()} 注入。
    from remote_tools import make_remote_tools
...
def main():
    ...
    print("🤖 交互式 Agent")
    print("=" * 64)
    print("命令：/save /rename /resume /list /show /reset /config /budget /stats /status /context /model /autonomous /memory /logs /download /web /snapshot /rewind /rag /tools /update /reload /restart /continue /debug /agent /help")
