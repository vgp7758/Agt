...
#### 方向澄清：为什么高 ratio 是「晚折叠」而非直觉的「更低更好」（2026-08）

**用户疑问（配置落地后）**：「DeepSeek 的折叠目标比率不应该是设置的更低一些才有利于避免频繁断缓存吗？」

**澄清：因果正好反过来**——断缓存的元凶**不是**投影大小 / 每轮追加量（追加在尾部、不影响前缀缓存），**是折叠本身改写了历史段头部前缀**：

```
ratio 低（如 0.5）→ 目标线低（700K×0.5=350K）→ 投影涨到线就触发折叠
  → 折叠改写历史段头部前缀 → 缓存从那里断 → 更频繁断缓存 ❌

ratio 高（如 0.95）→ 目标线高（665K）→ 折叠频率低 → 前缀稳定时间长 → 缓存更稳定 ✅
```

**价差账**：DeepSeek miss≈hit 的 30~60 倍——**宁可投影大（命中便宜 1x）也不要频繁折叠（未命中 60x）**。所以 0.95 正是对的：晚折叠、保前缀稳定；配 `detail_step: 0`（组内字节恒定）更完整——折叠频率最低 + 每组字节恒定 = 前缀物理极限稳定。

**直觉陷阱的来源**：也许在想「低目标线 = 投影小 = 每轮变化幅度小」——但缓存看的是**前缀是否字节稳定**，不是投影总量。投影 200K 全量追加（前缀不变）命中 99%；投影 600K 但每轮折叠一次（前缀改写）命中 0%——**大小不重要，稳定才重要**。
