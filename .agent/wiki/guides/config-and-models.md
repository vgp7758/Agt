...
| DeepSeek in-history system 能力位（2026-09-12，spec s_eb14a8fd，commit `1dc8829`） | `in_history_system: true`——SYSTEM 段变化走**尾部 append**（前缀完整命中，实测 94.3%）而非头部 replace（全断 0%）；仅实测背书的 deepseek / deepseek-chat 已标，其余默认 false=归一化（现状行为），anthropic 顶层 system 形态天然不支持。归一化时机=毕业/折叠断点处清账（用户裁定），机制见 [system 段 append-not-replace](../architecture/context-engine.md#system-段-append-not-replace2026-09-12) |
