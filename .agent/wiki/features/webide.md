...
- 首次启动要下载 server 组件（一次性 ~1 分钟）；此后 **`--commit-id` 用缓存秒起 + 滚动预热自动跟进新版**（2026-09-17 v1 钉版本 + 同日 v2 滚动预热：打开永远秒起、旧版自动清理、版本比 stable 最新晚一拍跟进，见下方专节及「滚动预热」小节）；12s 等待上限返回 `ready=false` **不是失败**——下载页自带自动刷新，页签开着等即可。
