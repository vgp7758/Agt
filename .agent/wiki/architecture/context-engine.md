...
```
低 ratio（如 0.5）→ target 低（350K）→ 投影更快超线 → 压力循环用【升档】就够压到低线
  → 折叠（头部大断）极少触发 → DeepSeek 60x 差价下缓存友好 ✅

高 ratio（如 0.95）→ target 高（665K，近窗口）→ 投影涨到近窗才超线，此时历史长期
  滚动毕业、多已退化为超深档（升档对它无效）→ 只能【折叠】（头部大断）❌
```

**所以 DeepSeek（miss≈hit 60x）配低 ratio（0.5）——更早让小步升档消化压力、避免累积成大折叠；GLM（4x）用默认 0.75 平衡点**。配 `detail_step:0`（不衰减）更进一步：组内字节恒定 + 折叠频率最低 = 前缀物理极限稳定。
